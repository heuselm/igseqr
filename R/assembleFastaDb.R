...
assembleFastaDb = function(IGSeq_resultset, dataset_tag = NULL, write_fasta = TRUE, write_fasta_per_sample = FALSE, write_clone_tables = TRUE, write_fasta_subset_cloneIdGlobal = NULL){
  ...
  ##########################################################
  IGSeq_resultset$step5_finalclones[, cloneIdGlobal:=paste0(dataset_tag, SAMPLE, "_", cloneId), .(cloneId, SAMPLE)]
  ...
  if (write_fasta){
    ...
    if(write_clone_tables){
      write.table(IGSeq_resultset$step6_clonestofasta[subset == TRUE], paste0(dataset_tag, "_final_clones_annotated.tsv"), sep = "\t", quote = F, row.names = F)
    }
    write.fasta(as.list(IGSeq_resultset$step6_clonestofasta[subset == TRUE]$aaSeq_fullVarDomain), as.string = TRUE,
                names = as.list(IGSeq_resultset$step6_clonestofasta[subset == TRUE]$fasta_header), file.out = paste0(dataset_tag, "_final_clones_annotated.fasta"))
    if (write_fasta_per_sample){
      ...
      for (i in seq_along(samples)){
        ...
        write.table(IGSeq_resultset$step6_clonestofasta[subset == TRUE][SAMPLE == samples[i]],
                    file = paste0(dataset_tag, samples[i], "_final_clones_annotated.tsv"),
        ...
        write.fasta(as.list(IGSeq_resultset$step6_clonestofasta[subset == TRUE][SAMPLE == samples[i]]$aaSeq_fullVarDomain),
                    ...
                    file.out = paste0(dataset_tag, samples[i], "_final_clones_annotated.fasta"))
      }
      }
    }
